...
@push('title')
    TokoDigi ID | Master Category
@endpush

@section('content')
<div class="p-4 pb-12 pt-20 sm:ml-72">
    <div class="my-4 text-center">
        <h2 class="text-xl font-extrabold leading-none tracking-tight text-gray-900 md:text-3xl lg:text-4xl dark:text-white">Master Category <span class="bg-blue-100 text-blue-800 text-2xl font-semibold mr-2 px-2.5 rounded dark:bg-blue-200 dark:text-blue-800 ml-2">Beta</span></h2>
                ...
                <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                    ...
                    <div class="px-6 py-6 lg:px-8">
                        ...
                        >
                            @csrf
                            <div>
                                <label for="name" class="block mb-1 text-sm font-medium text-gray-900 dark:text-white">Nama Kategori</label>
                                <div class="flex">
                                    ...
                                    <input type="text" id="name" name="name" class="rounded-none rounded-r-lg bg-gray-50 border border-gray-300 text-gray-900 focus:ring-blue-500 focus:border-blue-500 block flex-1 min-w-0 w-full text-sm p-2.5  dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                                        placeholder="ex: Top Up Games" required>
                                    </div>
                            </div>
                            <div>
                            ...
                            <div class="flex flex-col items-center justify-center w-full">
                                <label for="logo" class="block text-sm font-medium text-gray-900 dark:text-white text-left self-start">Foto / Logo Kategori</label>
...
<script>
    ...
    function fetchDataAndAppendToTable(params, pageUrl = "{{ route('dashboard.category.getData') }}") {
        // Make a GET request using Axios with the provided params
        // axios.get("{{ route('dashboard.category.getData') }}", {
        //     params: params
        // })
        axios.get(pageUrl, {
            params: params
            ...
            .then(function (response) {
                ...
                `
                    <tr class="bg-white text-center border-b dark:bg-gray-800 dark:border-gray-700">
                        ...
                        <td class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">${item.slug}</td>
                        <td class="px-6 py-4"><a href="${assetUrl}/logo/${item.logo}">${item.logo}</a></td>
                        <td class="px-6 py-4 text-center">
                            <button
                                class="relative inline-flex items-center justify-center p-0.5 mb-2 mr-2 overflow-hidden text-sm font-medium text-gray-900 rounded-lg group bg-gradient-to-br from-teal-300 to-lime-300 group-hover:from-teal-300 group-hover:to-lime-300 dark:text-white dark:hover:text-gray-900 focus:ring-4 focus:outline-none focus:ring-lime-200 dark:focus:ring-lime-800"
    ...
    function handleDeleteAction(id) {
        ...
        confirmButton.addEventListener('click', () => {
            ...
            axios
                .post(`{{ route('dashboard.category.delete') }}`, {
                    _method: 'DELETE',
                    id: id
                })
                .then(response => {
    ...
    function updateCategory() {
        ...
        // Assuming you have a route named 'dashboard.category.update' for updating categories
        axios.patch("{{ route('dashboard.category.update') }}", formData)
            .then(response => {
                // Handle the response (e.g., show a success message)
                console.log('Category updated successfully', response);
                fetchDataAndAppendToTable(); // Refresh the table after update
            })
            .catch(error => {
                // Handle any errors (e.g., show an error message)
