...
@section('content')
    <div class="max-w-screen-xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        <div class="flex flex-col">
            ...
            <table>
                <thead class="border-b border-b-black font-medium" style="margin-bottom: 8px;">
                    <tr>
                        <td class="text-center" style="padding: 8px; width: 3%">#</td>
                        <td style="padding: 8px;">Product Name</td>
                        <td class="text-left" style="padding: 8px; width: 17%">Ammount</td>
                ...
                <tbody class="text-gray-500">
                    ...
                    @foreach($cart->Details as $detail)
                        <tr>
                            <td class="text-black text-center" style="padding: 6px;">{{ ++$i; }}.</td>
                            ...
                            <td class="text-left" style="padding: 6px;">Rp {{ number_format($detail->Program->price * $detail->quantity) }}</td>
                            <td style="padding: 6px;">
                                <div class="flex flex-col">
                    ...
                    <tfoot class="border-t border-t-gray-400" style="margin-top: 8px;">
                        @php
                            $pfee = $total * 0.03;
                            $sfee = $total * 0.005 + 2000;
                            $sfee = 0;
                            $totalAll = $total + $pfee + $sfee;
                        @endphp
                        <tr>
                            <td style="padding: 2px;"></td>
                            <td style="padding: 2px;" class="text-gray-500 ">Program Price</td>
                            <td style="padding: 2px;"></td>
                            <td style="padding: 2px;" class="text-gray-500"><span class="">Rp {{ number_format($total); }}</span><span></span></td>
                            <td style="padding: 2px;"></td>
                        </tr>
                        <tr>
                            <td style="padding: 2px;"></td>
                            <td style="padding: 2px;" class="text-gray-500 italic"><span class="text-red-700">*</span> Payment Fee ( 3% * Total Ammount )</td>
                            <td style="padding: 2px;"></td>
                            <td style="padding: 2px;" class="text-gray-500 italic">+ Rp {{ number_format($pfee) }}</td>
                            <td style="padding: 2px;"></td>
                        </tr>
                        <tr>
                            <td style="padding: 2px;"></td>
                            <td style="padding: 2px;" class="text-gray-500 italic decoration-red-700"><span class="text-red-700">*</span> Service Fee ( 0.5% + Rp 2.000 )</td>
                            <td style="padding: 2px;"></td>
                            <td style="padding: 2px;" class="text-gray-500">
                                <span class="ml-1 italic font-medium"> Free</span>
                                <sup class="line-through decoration-red-700 italic text-xs suppers">+ Rp {{ number_format($sfee); }}</sup>
                            </td>
                            <td style="padding: 2px;"></td>
                        </tr>

                        <tr class="border-y border-y-gray-400 border-b-black font-medium" style="margin-top: 8px; margin-bottom: 12px;">
                            <td style="padding: 10px 6px;"></td>
                            <td style="padding: 10px 6px;" class="text-gray-500 italic">Total</td>
                            <td style="padding: 10px 6px;"></td>
                            <td style="padding: 10px 6px;" class="text-gray-500 italic">Rp {{ number_format($totalAll) }}</td>
                            <td style="padding: 10px 6px;"></td>
                        </tr>
                    </tfoot>
                </table>
    ...
    <button type="submit" hidden id="deleteButton" data-modal-toggle="checkoutModal" class="flex items-center justify-center text-white bg-gray-700 hidden hover:bg-gray-800 focus:ring-4 focus:outline-none focus:ring-gray-300 font-medium rounded-lg text-sm px-5 py-2.5 mr-5 text-center dark:bg-gray-600 dark:hover:bg-gray-700 dark:focus:ring-gray-800" type="button">
...
<script>
    ...
    function getData(params, pageUrl = "{{ route('cart.getData') }}") {
            ...
            .then(function (response) {
                ...
                if (data && data.Details.length > 0) {
                    ...
                    modalCheckout.innerHTML = `
                        <!-- Modal toggle -->
                        <div class="">
                            <form action="{{ route('checkout.pay') }}" method="POST" class="space-y-6" onsubmit="submitForm(event)">
                                @csrf
                                ...
                                <div class="flex justify-end m-5 w-full">
                                    <button type="submit" id="checkoutbutton" data-modal-toggle="checkoutModal" class="flex items-center justify-center text-white bg-gray-700 hover:bg-gray-800 focus:ring-4 focus:outline-none focus:ring-gray-300 font-medium rounded-lg text-sm px-5 py-2.5 mr-5 text-center dark:bg-gray-600 dark:hover:bg-gray-700 dark:focus:ring-gray-800" type="button">
                                        <svg class="w-4 h-4 mx-2 text-white dark:text-white" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 20 20">
                                            <path d="M11.074 4 8.442.408A.95.95 0 0 0 7.014.254L2.926 4h8.148ZM9 13v-1a4 4 0 0 1 4-4h6V6a1 1 0 0 0-1-1H1a1 1 0 0 0-1 1v13a1 1 0 0 0 1 1h17a1 1 0 0 0 1-1v-2h-6a4 4 0 0 1-4-4Z"/>
                                            <path d="M19 10h-6a2 2 0 0 0-2 2v1a2 2 0 0 0 2 2h6a1 1 0 0 0 1-1v-3a1 1 0 0 0-1-1Zm-4.5 3.5a1 1 0 1 1 0-2 1 1 0 0 1 0 2ZM12.62 4h2.78L12.539.41a1.086 1.086 0 1 0-1.7 1.352L12.62 4Z"/>
                                        </svg>
    ...
    function submitForm(event) {
        ...
        axios.post('{{ route('checkout.pay') }}', formData)
            .then(response => {
